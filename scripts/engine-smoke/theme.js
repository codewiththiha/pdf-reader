import { PDFReader, assertClose, created, expectedBakePixel, fakeComputed, setFakeComputed, getEl, } from "./harness.js";
export async function run() {
    ...
    if (!r2.ok)
        throw new Error("render2 failed: " + JSON.stringify(r2));
    const darkAllocs = created.length - beforeDark;
    if (darkAllocs < 1)
        throw new Error("dark bake should allocate at least one intermediate canvas, got " + darkAllocs);
    const cv1 = getEl("cont-1-cv");
    ...
    if (scrubPx[0] < 200 || scrubPx[1] < 200 || scrubPx[2] < 200) {
        throw new Error("scrub should show raw page pixels, got [" +
