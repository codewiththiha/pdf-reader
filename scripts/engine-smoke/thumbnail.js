import { PDFReader, assertClose, expectedBakePixel, fakeComputed, setFakeComputed, getEl, } from "./harness.js";
export async function run() {
    // 6. thumbnails
    const t = await PDFReader.renderThumb("thumb-1", 1, 0.25);
    if (!t.ok)
        throw new Error("thumb failed: " + JSON.stringify(t));
    console.log("thumb ok:", t.width, t.cached);
    const t2 = await PDFReader.renderThumb("thumb-1", 1, 0.25);
    if (!t2.ok || t2.cached !== true)
        throw new Error("thumb cache hit failed");
    ...
    const liveThumb = getEl("thumb-1");
    const liveThumbPx = liveThumb._ctx.getImageData(0, 0, 1, 1).data;
    const liveThumbExpect = expectedBakePixel([255, 255, 255], fakeComputed["--canvas-filter"], "screen", [19, 19, 22]);
    ...
    if (!t3.ok)
        throw new Error("thumb after theme change failed: " + JSON.stringify(t3));
    const t4 = await PDFReader.renderThumb("thumb-1", 1, 0.25);
    if (!t4.ok || t4.cached !== true)
        throw new Error("rebaked thumb should hit cache, got " + JSON.stringify(t4));
