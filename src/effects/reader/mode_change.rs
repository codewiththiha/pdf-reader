...
//! is one write to `viewer.mode`, and four things follow it:
//!
...
//!   scroll→page sync stands down for that flush;
...
//! * the fit is the next mode's to resolve: reinterpreting the outgoing
//!   layout's fit against a new axis is a zoom jump, so the flip hands
//!   ownership over instead.
//!
//! Installed by the reader page, after the layout prefs and reflow effects and
//! before the zoom controller.
...
pub fn mode_change(state: AppState) {
    ...
    Effect::new(move |_| {
        ...
        // itself to `viewer.page` in `ScrollShell`; until then its dominant is
        // not the reader's page. Raised HERE, in the same flush as the flip,
        // so the scroll→page arm that re-runs for it stands down rather than
        // reading the unplaced strip.
        ...
        // stream has no page to fit — the window is the page and type size
        // belongs to the typography settings — so a fit resolved against the
        // A4 page model would only shrink the text below its setting. The
        // paged modes re-resolve their own fit on entry (branch below), so
        // nothing needs restoring on the way out.
        ...
        if mode == ViewMode::ScrollHorizontal {
            ...
            // the outgoing layout's fit against the new axis: a single/
            // vertical width fit would become a height fit here and drop the
            // readout by almost half; a spread width fit would jump the other
