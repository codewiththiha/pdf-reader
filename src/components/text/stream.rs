...
use text_core::pager::first_block_of_page;
...
pub fn TextStreamLayout(
    ...
    let block_count = Signal::derive(move || {
        state.text.doc.track();
        state.text.heights.with(|h| h.len())
    });
    ...
    let epoch = Signal::derive(move || {
        ...
        if let Some(doc) = state.text.doc.get() {
            (Arc::as_ptr(&doc) as usize).hash(&mut hasher);
        }
        let heights = state.text.heights.get();
    ...
    // the stream through `state.text.stream` rather than a second wiring.
    state.text.stream.set_value(Some(v.clone()));
    {
        on_cleanup(move || state.text.stream.set_value(None));
    }
    ...
    // can never enter.
    {
        let mirror = state.text.stream_total;
    ...
    let column_ref: NodeRef<html::Div> = NodeRef::new();
    {
        ...
        Effect::new(move |_| {
            ...
            let _epoch = state.text.remeasure.get();
    ...
    let column_style = move || {
        let s = scale.get();
        let geo = text_core::page::geometry(typography.get().book_layout);
        let m = margin.get();
        format!(
    ...
    let progress = move || {
        ...
        let total = state.text.stream_total.get();
    ...
    view! {
        <div class="relative h-full w-full">
            <div
                id=PAGE_LIST_ID
                node_ref=list_ref
                class="tx-stream scrollbar-none h-full w-full overflow-y-auto outline-none"
                tabindex="0"
            >
                <div class="relative w-full" style:height=move || format!("{}px", total_size.get())>
                    <div node_ref=column_ref class=column_class style=column_style>
                        <For
                            each=move || {
                                let doc_id = doc_identity(state.text);
                                items
                                    .get()
                                    .into_iter()
                            ...
                            children=move |(_, item): (usize, VirtualItem)| {
                                ...
                                let block = block_at(state.text, index);
                                view! {
                                    <div
                                        class="tx-content"
                                        lang="en"
                                    ...
                                    >
                                        {match block {
                                            Some(block) => {
                                                view! { <TextBlockView block=block /> }.into_any()
                                            }
...
/// One block by index, read untracked at child creation (the key carries
/// the document identity, so a new document remounts its children).
fn block_at(text: TextDocState, index: usize) -> Option<TextBlock> {
    text.doc
        .with_untracked(|doc| doc.as_ref().and_then(|d| d.blocks.get(index).cloned()))
}

/// The open document's identity for the item keys: the Arc pointer. Keying
/// the window on it means a different document remounts every block instead
/// of reusing index keys the outgoing file already occupied.
fn doc_identity(text: TextDocState) -> usize {
    text.doc
