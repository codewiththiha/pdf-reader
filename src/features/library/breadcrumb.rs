...
//! always a button, because it is the way back. The shelf crumb is a button too,
//! but for a different reason: it is where a shelf the reader made gets its name
//! changed or gets taken apart, and a menu hanging off the crumb is the one place
//! on the page where "this shelf" has a handle.
//!
...
//!
//! Nothing here captures a shelf id. Every action asks which shelf the page is on
//! when it runs, which is both the honest question — a crumb is a handle for
//! *here*, and "here" can change under a menu that is still open — and the reason
//! these closures can all be `Fn`: a reactive child is re-run, and a closure that
//! moved an id out of its environment on the first run has nothing left for the
//! second.
...
/// The shelf the page is drilled into, as `(name, is the reader's own)`. `None` at
/// the root. Tracked: the breadcrumb is a view, and a drill in or out is one of the
/// two things that change it.
fn crumb(state: AppState) -> Signal<Option<(String, bool)>> {
    Signal::derive(move || {
        ...
        state
            .library
            .shelves
            .with(|shelves| shelves.iter().find(|s| s.id == id).map(|s| (s.name.clone(), !s.is_folder())))
    })
}

#[component]
pub(crate) fn Breadcrumb(state: AppState) -> impl IntoView {
    let crumb = crumb(state);
    // One set of handles, shared with whatever shelf is drilled into: only one crumb
    // exists at a time, so a rename or a menu cannot be open in two places at once,
    // and a fresh crumb inherits a closed menu rather than a stale open one.
    let menu_open = RwSignal::new(false);
    let renaming = RwSignal::new(false);
    let draft = RwSignal::new(String::new());
    let anchor: NodeRef<html::Div> = NodeRef::new();
    let at_root = Signal::derive(move || crumb.get().is_none());
...
/// The second crumb: the chevron, the shelf's name, and the menu behind it.
///
                ...
                view! {
                    <div node_ref=anchor class="relative flex min-w-0 shrink items-center">
                        <button
                            ...
                            class="flex min-w-0 items-center gap-1 rounded-md px-1.5 py-0.5 font-medium \
                                   text-ink transition-colors hover:bg-line focus:outline-none \
