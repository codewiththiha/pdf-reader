...
use library_core::book::{
    Book, Origin, Row, book_rows, drop_dangling_links, duplicate_title, find_book_mut, find_row,
    remove_row,
};
...
///
/// Crate-visible because a departure is never the only conversion: the import
/// side flips rows too, and every flipped row owes its marks the same move.
    ...
    fn of(
        ...
        for id in departing {
            ...
            let folder_name = folder
                .map(|f| super::folder_label(&f.root))
...
fn return_path(
    ...
    (one.parent != seat).then(|| ReturnPath::Reseat {
        seat,
        family_name: super::folder_label(&folder.root),
    })
}
...
pub fn answer_departure_return(state: AppState) {
    ...
    for ret in &ask.returns {
        let moved = match &ret.path {
            ReturnPath::Reclaim {
                ...
                rel,
                ..
            } => super::import::reclaim_rung(state, tree, gone, rel, &ret.shelf_id),
    ...
    if let Some(id) = first {
        super::reveal::reveal_shelf(state, &id);
    }
}
