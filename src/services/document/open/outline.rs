...
//! destination through the pdf.js worker — a per-entry round trip textbook
//! outlines pay in seconds — and none of it is needed to paint the first page.
//! Asking here keeps `open` fast and the reader mounts the moment page 1 is
//! known; the panel shows a resolving state (`outline_pending`) and fills when
//! the tree is back.
//!
//! This is the PDF tail. A reflowable document's chapters are already in its
//! text: its outline is projected from the block list by
//! `effects::reader::reflow_outline`, and the open flow files it without a hop
//! through here.
...
pub(super) fn resolve(state: AppState, path: String, stamp: u64) {
    // The clamp needs the page count, which is the open flow's answer and
    // already published (outline resolution starts after `Ready`). Read
    // untracked: a one-shot write, not a subscription.
