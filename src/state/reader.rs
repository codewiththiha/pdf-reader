...
///
/// There is deliberately no position in here. The strips are relaid out on
/// every frame of the tween and the virtualizer's own rescale anchor keeps
/// the reader's view steady as the sizes underneath it move, so a
/// transaction has nothing to remember about where the reader was looking.
...
///   explains it).
/// - `display` is the live visual scale. Page hosts stretch their bitmaps to
///   it and the strips are relaid out to it, frame by frame, for the whole
///   zoom. It is what the readout, the fit maths and the overlays read.
...
pub struct ZoomState {
    ...
    /// The live visual scale. Pages stretch to it and the strips relayout to
    /// it on every frame of a zoom.
