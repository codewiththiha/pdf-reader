...
//! they carry, one class name, and the shape of element ids. Neither compiler
//! sees the other, so a rename here is not an error there — it is a query that
//! quietly returns nothing.
//!
//! What lives here is the half Rust uses as a *value*: in a `closest`
//! selector, a `get_attribute`, a comparison. Two more names cross the
//! boundary and cannot live here, because Leptos takes an attribute's NAME
//! from the markup and only its value from an expression:
//!
...
//!   `crate::components::formats::reflow::stream`. The first thing the
//!   engine's selection tracker asks a host for; the id shape is only the
//!   fallback.
//! * `data-ai-popover` — marks the AI pill's root so a press inside it is not
//!   read as a click that clears the selection
//!   (`crate::components::ai::selection_pill`).
//!
//! Element ids are built, not spelled:
//! `crate::components::viewer::page_host` owns
//! [`crate::components::viewer::page_host::host_id_for_mode`] (the
...
//! the strings are not here.
//!
//! The engine's half is `public/engine/dom-contract.ts`, and
//! `tools/check-dom-contract.ts` fails CI when the halves disagree — reading
//! the id shapes out of the builders' `format!` strings rather than trusting a
//! copy, so a rename in either language is caught by the other.
...
/// the block number off it — which is what makes the paginated modes and the
...
/// div (`crate::components::formats::pdf::canvas`); the engine fills it with
