//! Typography for reflowable documents (plain text and Markdown).
//!
//! This is the persisted settings schema for the text formats, in the same
//! sense that `pdf_core::settings` is the schema for the reader at large:
//! the field names below ARE the storage contract, so they are additive
//! only — every field carries `#[serde(default)]` semantics through the
//! struct-level default, and a blob saved before a field existed loads with
//! that field's default.
//!
//! Two layers:
//!
//! * the SCHEMA — [`TextSettings`], what the reader persists, plus
//!   [`sanitize`] clamping into the supported ranges;
//! * the RESOLUTION — fonts resolve to CSS font stacks ([`resolve_stack`])
//!   and the whole setting resolves to CSS custom properties
//!   ([`css_variables`]), which is the one bridge the interface uses.
//!
//! Fonts are chosen from the system's faces today; [`BuiltInFont`] /
//! [`builtin_fonts`] are the deliberately-empty extension point a future
//! release fills with fonts shipped inside the app. The settings schema
//! already serialises them (`builtin:<name>`), so adding one is adding a
//! row to the table, not a migration.

use serde::{Deserialize, Serialize};

/// The three CSS font families the reader models. Every font face belongs
