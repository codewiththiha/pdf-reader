...
pub fn ui_hue_oklch(srgb_hue: f64) -> f64 {
    ...
    crate::color::oklch::hex_to_oklch(&hex)
        .map(|(_, _, h)| h)
        .unwrap_or(srgb_hue)
}
...
impl Appearance {
    ...
    pub fn tinted_accent(&self) -> Option<String> {
        ...
        let hex = self
            ...
            .map(|(_, v)| v)?;
        let (l, c0, h0) = crate::color::oklch::hex_to_oklch(hex)?;
        ...
        Some(crate::color::oklch::oklch_css(l, c, h))
    }
    ...
    pub fn ui_overrides(&self) -> Vec<(&'static str, String)> {
        ...
        for (name, base_var, max_c) in tokens {
            ...
            let Some((l, c0, h0)) = crate::color::oklch::hex_to_oklch(hex) else {
                continue;
            };
            ...
            out.push((name, crate::color::oklch::oklch_css(l, c, h)));
        }
        out
    }
    ...
    fn the_tint_preserves_each_tokens_lightness_exactly() {
        ...
        for strength in [10u8, 50, 90, 100] {
            ...
            for (token, base_hex) in [
                ...
                let want = crate::color::oklch::hex_to_oklch(base_hex).unwrap().0;
