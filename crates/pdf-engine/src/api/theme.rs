...
/// variables every frame, the engine shows the RAW rasters under the live CSS
/// filter/blend so the page re-colours per frame; leaving re-bakes from the
/// raws. The engine swaps canvas contents and the CSS class in the same task,
/// so no frame is ever double-filtered or unfiltered.
...
/// change, leaving plain opaque textures). The engine swaps the rasters for
/// the new mode; the caller only states the preference.
